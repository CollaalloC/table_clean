...
# 路径设置
# 注意：这里假设 assets 文件夹在 table_clean 目录下，即 table_clean_task 的上一级目录
# 如果 assets 在 table_clean_task 内部，请调整路径
...
@configclass
class TableCleanIKEnvCfg(LiftEnvCfg):
    """
    Table Clean Environment with IK Control.
    Inherits from LiftEnvCfg but replaces the scene and robot configuration.
    """
    def __post_init__(self):
        super().__post_init__()

        # ---------------------------------------------------------
        # 1. 机器人配置 (参考 ik_rel_env_cfg.py)
        # ---------------------------------------------------------
        # 使用 High PD 配置，这对 IK 跟踪很重要
        ...
        # 禁用 object_pose 命令的重采样，防止在遥操作中目标点跳变
        self.commands.object_pose.resampling_time_range = (1.0e9, 1.0e9)

        # ---------------------------------------------------------
        # 3. 场景物体配置 (参考 table_clean_scene.py)
        # ---------------------------------------------------------
        ...
        # C. 随机杂货 (作为操作对象)
        # 注意：LiftEnvCfg 默认只处理一个 object。
        # 这里我们使用 MultiAssetSpawnerCfg 来随机生成一种物品。
        self.scene.object = RigidObjectCfg(
            ...
            spawn=MultiAssetSpawnerCfg(
                ...
                random_choice=True, # 每次 reset 随机选一个
            ),
        )
